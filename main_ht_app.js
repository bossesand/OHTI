...
function BluetoothGo(noble) {
  ...
  this.ble.on("discover", (peripheral) => {
    ...
    this.bleUnitsFound[peripheral.advertisement.localName] = {
      unit: peripheral,
    };
  ...
  if (devicePheriperal.advertisement.localName == "OHTI") {
    console.log(
      "Trying to Connect with " +
        devicePheriperal.advertisement.localName +
        " unit: ",
      devicePheriperal.advertisement
    );
    ...
    devicePheriperal.connect((error) => {
      if (error) {
        ...
        console.log(
          "\x1b[36m%s\x1b[0m",
          "Connected to UUID: ",
          devicePheriperal.uuid
        );
        this.bleDeviceState = "connected";
        this.bleDeviceConnectedTo = {
          uuid: devicePheriperal.uuid,
          localname: devicePheriperal.advertisement.localName,
          subscription: false,
        };
        this.emit();

        devicePheriperal.discoverAllServicesAndCharacteristics(
          (error, services, characteristic) => {
            ...
            for (var y = 0; y < services.length; y++) {
              ...
              for (var x = 0; x < services[y].characteristics.length; x++) {
                console.log(
                  "Service Name: " +
                    services[y].characteristics[x].name +
                    ", UUID: " +
                    services[y].characteristics[x].uuid +
                    ", Type: " +
                    services[y].characteristics[x].type +
                    ", Properties: " +
                    services[y].characteristics[x].properties
                );
                ...
                services[y].characteristics[x].on(
                  "data",
                  function (data, isNotification, more) {
                    if (typeof data != "undefined" && data != null) {
                      if (isNotification) {
                        let stringData = data.toString();
                        let stringDataNoWhite = stringData.replace(
                          /^\s+|\s+$/g,
                          ""
                        );
                        ...
                        if (stringDataNoWhite.length > 0) {
                          ...
                          // Little endian
                          let quat16b1 = parseFloat(
                            dataDecode.readInt16LE(0, true) / 16384
                          );
                          let quat16b2 = parseFloat(
                            dataDecode.readInt16LE(2, true) / 16384
                          );
                          let quat16b3 = parseFloat(
                            dataDecode.readInt16LE(4, true) / 16384
                          );
                          let quat16b4 = parseFloat(
                            dataDecode.readInt16LE(6, true) / 16384
                          );
                ...
                if (services[y].characteristics[x].properties == "notify") {
                  ...
                  tmpCharacteristic.subscribe(function (error) {
                    ...
                    console.log(
                      "Notification on characteristics is on, Roger check if this is sent before data starts coming in from sensor..."
                    );
                  });
                }
              }
...
BluetoothGo.prototype.stopCharacteristics = function () {
  ...
  this.bleSubscribedCharacteristics.forEach(function (element) {
    element.unsubscribe(function (error) {
      if (error) {
        console.log(
          "Error: Unsubscribing to notification on characteristics is unchanged"
        );
        console.dir(error);
      } else {
        that.bleDeviceConnectedTo.subscription = false;
  ...
  this.bleSubscribedCharacteristics.forEach(function (element) {
    element.subscribe(function (error) {
      if (error) {
        that.bleDeviceConnectedTo.subscription = false;
        console.log(
          "Error: Subscribing to notification on characteristics is unchanged"
        );
        console.dir(error);
      } else {
        that.bleDeviceConnectedTo.subscription = true;
...
io.on("connection", function (Socket) {
  ...
  Socket.on("usb-start-track", (msg) => {
    // User starts tracking
    console.log(
      "> Starting Serial USB watch: " + msg + ", on port: " + USERPORT
    );
  ...
  Socket.on("usb-stop-track", (msg) => {
    // User stops tracking
    console.log(
      "> Stopping Serial USB watch: " + msg + ", on port: " + USERPORT
    );
    if (SerialObj !== null) {
      SerialObj.stop();
    } else {
