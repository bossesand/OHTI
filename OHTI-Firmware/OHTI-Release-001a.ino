...
#define PIN_LED1                (17) // RGR
#define PIN_LED2                (19) // RGR
#define PIN_LED3                (31) // RGR var definierat direkt pa status_led forut

#define LED_BUILTIN             PIN_LED1 // RGR
#define STATUS_LED              PIN_LED3
...
float   = sqrt(3.0f / 4.0f) * GyroMeasError;   // compute  
...
// ==========================================================
// Bluefruit Advertising Packet Setup
void initBLEAdvertising(void)
{
  ...
  Bluefruit.Advertising.start(0);                // 0 = Don't stop advertising after n seconds, uint8_t mode = 0
}
...
void setup() {
  ...
  //bleuart.begin(); // RGR Second time start

  // Set up Advertising Packet and start advertising
  initBLEAdvertising();
  ...
  bleuart.println("BNO055 ReadByte starts");
  ...
  if (c == 0xA0) { // BNO055 WHO_AM_I should always be 0xA0  
    ...
    //bleuart.print("BNO055 SW Revision ID: "); bleuart.print(swmsb, HEX); bleuart.print("."); bleuart.println(swlsb, HEX); bleuart.println("Should be 03.04");
    ...
    bleuart.print("System calibration status (Not calibrated = 0, fully calibrated = 3) "); bleuart.println( (0xC0 & calstat) >> 6);
    ...
    initBNO055(); 
    bleuart.println("BNO055 initialized for sensor read mode.");
  }
  else
  {
...
void loop() {
  ...
  //dtostrf((float)(quatCount[0])/16384., 15, 8, htQuatCombo.selva);
  ...
  */
  ...
  size_t bytesEncoded = Z85_encode(htQuatCombo.selva, encBuf, 8);
...
void magCalBNO055(float * dest1) 
{
  ...
  bleuart.println("MagCal8"); // Mag Calibration: Wave device in a figure eight until done!
...
void disconnect_callback(uint16_t conn_handle, uint8_t reason)
{
  (void) conn_handle;
  (void) reason;
  bleuart.println("Disconnected, Bluefruit will auto start advertising (default)");
}